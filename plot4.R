#Script for Plot 4 - Electric Power Consumption
library(dplyr)
library(lubridate)
...
par(mar = c(5,5,2,2))
par(mfrow = c(2,2))
with(rel.dat,plot(ymd_hms(Date),Global_active_power,type="l",ylab = "Global Active Power",xlab=""))
with(rel.dat,plot(ymd_hms(Date),Voltage,type="l",ylab = "Voltage",xlab="datetime"))
with(rel.dat,
...
with(rel.dat,plot(ymd_hms(Date),Global_reactive_power,type="l",xlab="datetime"))
dev.off()